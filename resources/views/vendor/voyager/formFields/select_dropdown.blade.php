@if(isset($options->relationship))
    ...
    @if( method_exists( $dataType->model_name, camel_case($row->field) ) )
        ...
        <select class="form-control select2" name="@if(isset($row->name)){{ old($row->field, $row->name) }}@else{{$row->field}}@endif">
    ...
    @else
        <select class="form-control select2" name="@if(isset($row->name)){{ old($row->field, $row->name) }}@else{{$row->field}}@endif"></select>
    @endif
@else
